...
if [ "${CP_OR_JAR}" == "cp" ]; then
  ${NOHUP} ${JVM} ${JVM_OPT} ${JVM_DBG} ${JVM_LOG} ${APP_OPT} ${JVM_PARAMS} -cp "${APP_CLASSPATH}" ${APP_MAIN} ${APP_PARAMS} &>> ./logs/${NOHUP_FILE} &
else
  ${NOHUP} ${JVM} ${JVM_OPT} ${JVM_DBG} ${JVM_LOG} ${APP_OPT} ${JVM_PARAMS} -jar *.jar &>> ./logs/$NOHUP_FILE &
fi
